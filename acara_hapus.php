...
if(isset($_GET['id'])){
	...
	$cek = mysqli_query("SELECT id_acara FROM acara WHERE id_acara='$id'") or die(mysql_error());
	...
	if(mysql_num_rows($cek) == 0){
		...
		$del = mysql_query("DELETE FROM acara WHERE id_acara='$id'");
		
		//jika query DELETE berhasil
		if($del){
			
			echo 'Data siswa berhasil di hapus! ';		//Pesan jika proses hapus berhasil
			echo '<a href="acara.php">Kembali</a>';	//membuat Link untuk kembali ke halaman beranda
